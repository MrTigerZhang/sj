...
****************************************************************************************/
/**
 * 打开一个MDA组文件并将其内容加载到内存中。
 *
 * 参数：
 * - filename: 文件名，指向要打开的MDA文件的路径。
 * - handle: 指向整型变量的指针，用于返回分配给该MDA组的句柄。
 *
 * 返回值：
 * - TTN_OK: 成功打开并加载文件。
 * - TTN_NOT_OK: 发生错误（如文件不存在、不是有效的MDA文件、内存分配失败等）。
 */
...
SLONG read_mda_frame(UCHR *mda_filename,SLONG frame_no,UCHR **buffer)
{
	...
	fseek(fp,temp_mda_head.data_start_offset,SEEK_SET);	//SEEK_SET=0 �ƶ������ݿ�ʼ��λ��
...
SLONG npc_auto_follow_move(SLONG idx)
{
	...
	if((map_npc_idx<0)||(map_npc_idx>=total_map_npc))	// �������뵱ǰ
	{
		return(TTN_NOT_OK);
	}
...
void add_baobao_show(CLIENT_BAOBAO_SHOW baobao_data)
{
	...
	map_npc_idx=get_map_npc_index(follow_id);
	if((map_npc_idx<0)||(map_npc_idx>=total_map_npc))			// �����Ѿ����ڵ�ǰ��Ļ
		return;
...
//sprintf((char *)print_rec,"���� command send (%d->%u)",character_fight_command_data.command,character_fight_command_data.target_id);
//display_system_message(print_rec);
	...
	memcpy(&backup_character_fight_command_data,&character_fight_command_data,sizeof(struct FIGHT_COMMAND_DATA_STRUCT));	// ��������
...
//sprintf((char *)print_rec,"���� command send (%d->%u)",baobao_fight_command_data.command,baobao_fight_command_data.target_id);
//display_system_message(print_rec);
	...
	memcpy(&backup_baobao_fight_command_data,&baobao_fight_command_data,sizeof(struct FIGHT_COMMAND_DATA_STRUCT));	// ��������
